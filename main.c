...
void draw_filled_circle(float x, float y, float rad, float r, float g, float b, float a){
...
void draw_ring_segment(float x, float y, float outerR, float innerR, float start_angle, float end_angle, float r, float g, float b, float a){
    ...
    draw_filled_circle(x + cos(start_rad) * mid_r, y + sin(start_rad)* mid_r, cap_r, r, g, b, a);
    float end_rad = (end_angle - 90.0f) * M_PI / 180.0f;
    draw_filled_circle(x + cos(end_rad) * mid_r, y + sin(end_rad)* mid_r, cap_r, r,g,b,a);
}

void update_cached_text(TTF_Font * font, CachedText * cache, const char * text, SDL_Color color){
...
void draw_cached_text(CachedText * cache, float x, float y, bool center, float alpha){
    if (cache->texture == 0)
        return;
...
int main(int argc, char *argv[]){
    ...
    SDL_Window *window = SDL_CreateWindow(
        "pomopomo", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, ww, wh, SDL_WINDOW_OPENGL | SDL_WINDOW_SHOWN);
    ...
    while(running){
        ...
        //back ringy
        draw_ring_segment(ww/2.0f, wh/2.0f, 160, 150, 0, 360, 0.15f, 0.17f, 0.25f, 1.0f);
        ...
        draw_ring_segment(ww / 2.0f, wh/2.0f, 160, 150, 0, 360.0f * progress, ring_r, ring_g, ring_b, 1.0f);
